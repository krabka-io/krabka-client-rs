//! Shared helpers for the container-driven admin suites.
//!
...
pub fn init_tracing() {
    let _ = tracing_subscriber::fmt()
        .with_env_filter("krabka_client_admin=debug,krabka_client_core=debug,info")
        .with_test_writer()
        .try_init();
}
...
///
/// One container serves one case here, but a case may create several topics
/// and groups, and a retry of a case must not read the state of the previous
/// attempt. The counter also keeps the names readable in broker logs.
...
///
/// A suite that needs a broker setting the default image does not carry -- a
/// second `log.dirs` entry, for one -- builds the image with
/// `testcontainers::ImageExt` and passes it here.
...
pub async fn bootstrap_client(bootstrap: &str) -> Client {
    for attempt in 1..=BOOTSTRAP_MAX_ATTEMPTS {
        let client = Client::builder()
            .bootstrap(bootstrap)
            .client_id("krabka-admin-integration")
            .build()
            .await
...
/// Connect an `AdminClient`, with the same warm-up retry as
/// [`bootstrap_client`].
pub async fn admin(bootstrap: &str) -> AdminClient {
    let addrs = [bootstrap.to_owned()];
    for attempt in 1..=BOOTSTRAP_MAX_ATTEMPTS {
        match AdminClient::connect(&addrs).await {
            Ok(admin) => return admin,
            Err(e) if attempt < BOOTSTRAP_MAX_ATTEMPTS => {
                tracing::warn!(attempt, "admin connect failed while broker warms up: {e}");
                warm_up_pause(attempt).await;
            }
            Err(e) => panic!("admin connect failed: {e}"),
        }
    }
...
/// Create one topic through the admin client and assert the broker accepted it.
pub async fn create_topic(admin: &mut AdminClient, name: &str, partitions: i32) {
    let outcomes = admin
        .create_topics(
            &[CreateTopicSpec {
                name: name.to_owned(),
    ...
    assert2::assert!(outcomes[0].error.is_none(), "{outcomes:?}");
}
