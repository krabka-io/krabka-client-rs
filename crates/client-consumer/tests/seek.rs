...
//!
//! The suite runs against a real broker. It proves three properties:
//!
...
//! - A seek on a partition that the consumer does not hold fails with
//!   `ConsumerError::NoCurrentAssignment`, as Kafka's `seek` throws
//!   `IllegalStateException`. A second group member is what makes a partition
//!   unassigned at the time of the seek.
...
/// A seek on a partition that the consumer does not hold fails.
///
/// `Consumer::build` returns only after `JoinGroup`, `SyncGroup` and the initial
/// offset prime, so a second member is what leaves a subscribed partition
/// unassigned. Kafka's `SubscriptionState.assignedState` throws
/// `IllegalStateException` for that seek.
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
#[ignore = "requires Docker"]
async fn seek_on_an_unassigned_partition_fails() {
    support::init_tracing();
    let kafka = support::start_kafka().await;
    let bootstrap = kafka.bootstrap.clone();
    ...
    let rejected = m1.seek(topic.as_str(), other, 2).await;
    assert!(let Err(ConsumerError::NoCurrentAssignment { partition, .. }) = rejected);
